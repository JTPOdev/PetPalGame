...
public class CookedSalmonDND : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.CompareTag("Mouth"))//tag for image box
        {
            ...
            hungry.SetActive(true);
            foodAnim.SetTrigger("CookedSalmon Ate");
    ...
    private void OnTriggerExit2D(Collider2D collision)
    {
        if (collision.CompareTag("Mouth"))//tag for image box
        {
            foodAnim.SetTrigger("CookedSalmon Idle");
    ...
    IEnumerator CountDownToStart()
    {
        ...
        foodAnim.SetTrigger("CookedFood Idle");
